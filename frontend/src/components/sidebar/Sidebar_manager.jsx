...
import { 
  ...
  TrendingUp, Settings, ChevronRight, X, LogOut
} from 'lucide-react';
...
// ==========================================
// 1. THE RBAC DICTIONARIES (From PDF)
// ==========================================
// Who is allowed to see the module AT ALL? (Full ✅ AND Viewers 👁)
const moduleAccess = {
  'Dashboard': ['Super Admin', 'Company Admin', 'Project Manager', 'HR', 'Team Lead', 'Viewers'], // Fixed
  ...
  'Margin Tracker': ['Super Admin', 'Company Admin', 'Project Manager', 'HR', 'Team Lead', 'Viewers'], // Fixed
  ...
  'Invoicing': ['Super Admin', 'Company Admin', 'Project Manager', 'Viewers'], // Fixed
...
// Notice 'Viewers' is completely absent here, forcing them into Read-Only mode!
const writeAccess = {
  'Organization': ['Super Admin', 'Company Admin'],
...
const masterMenuItems = [
  ...
  },
  {
    ...
    children: [
      ...
      { label: 'Add Cost', path: '/employee-cost/add', isAction: true },
...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Container - Glassmorphism enabled */}
      <aside className={cn(
        "fixed inset-y-0 left-0 w-64 bg-slate-950/40 backdrop-blur-xl border-r border-slate-800/50 h-screen flex flex-col z-40 transition-transform duration-300 lg:sticky lg:top-0 lg:translate-x-0",
              ...
              {item.path ? (
                ...
                <div>
                  <button
                    ...
                    className={cn(
                      "w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm font-bold text-slate-400 hover:bg-slate-800/50 hover:text-slate-100 transition-colors",
                      openMenus.includes(item.label) && "bg-slate-900/40 text-slate-200"
                    )}
                  >
                  ...
                  {openMenus.includes(item.label) && (
                    <div className="mt-1 ml-9 space-y-1 border-l border-slate-800/50 pl-2">
        ...
        <div className="p-4 border-t border-slate-800/50 bg-slate-950/30 space-y-2">
          <NavLink
            to="/settings"
            className={({ isActive }) => cn(
              "flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-bold transition-colors",
